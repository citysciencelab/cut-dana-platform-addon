        ...
        <div>
            <ol>
                <li v-for="(chapter, cIdx) in chapters" :key="chapter.id">
                    ...
                    <ol>
                        <li v-if="!chapter.steps?.length">Keine Schritte</li>

                        <li v-for="(step, sIdx) in chapter.steps" :key="step.id">
                            <div><strong>Schritt {{ sIdx + 1 }}:</strong> {{ step.title || "Ohne Titel" }}</div>
                            <ul>
                                <li v-if="step.description">{{ step.description }}</li>
